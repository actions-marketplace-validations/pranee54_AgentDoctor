...
| Surface                         | Notes                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fix`                           | Safe context exclusions: Cursor `.cursorignore`, Claude Code `permissions.deny` Read, Codex filesystem deny, Gemini `.geminiignore`, Aider `.aiderignore`; Copilot/Windsurf skipped (no official deny writer); review/manual skipped. Multi-writer apply preflights all targets then writes sequentially; `partial` is reported if a later write fails (no cross-file rollback). |
...
| `applyFixPlan` options           | Public contract is `{ dryRun: boolean }`. `simulateWriteFailureAt` is an internal test-only option type — not a CLI flag and not a supported consumer workflow |
