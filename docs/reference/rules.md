...
- **Agents:** Cursor only if not covered by `.cursorignore` / `.gitignore` / documented default `.env*` ignore; Claude Code unless a Read deny exists; Codex unless a filesystem deny exists in `.codex/config.toml`; Gemini CLI unless covered by `.geminiignore` / `.gitignore`; Aider unless covered by `.aiderignore` / `.gitignore`; Copilot / Windsurf attributed when detected (no official project deny Fix writer)
...
- **Ignores:** Root and nested `.gitignore` / `.cursorignore` / `.geminiignore` / `.aiderignore`, including `build/` and `**/build/`; Claude Code `permissions.deny` Read rules (`Read(./path/**)`); Codex filesystem deny keys in `.codex/config.toml`
- **Fix:** `agentdoctor fix` can append `.cursorignore`, `.geminiignore`, and `.aiderignore` patterns; Claude Code Read deny rules; and/or Codex filesystem deny keys. Copilot and Windsurf have no official project deny Fix writer.
